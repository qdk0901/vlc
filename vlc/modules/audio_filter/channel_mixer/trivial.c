 ...
 * Trivially upmixes
 */
#define MM_STEREO_TO_STEREO 0
#define MM_LEFT_CHANNEL_TO_STEREO 1
#define MM_RIGHT_CHANNEL_TO_STEREO 2
...
static block_t *Upmix( filter_t *p_filter, block_t *p_in_buf )
{
    ...
    int music_mode = var_InheritInteger( p_filter, "music-mode" );

    for( size_t i = 0; i < p_in_buf->i_nb_samples; i++ )
    {
	if (i_output_nb == 4 && i_input_nb == 2) {
		//special case

		switch (music_mode) {
			case MM_STEREO_TO_STEREO:	
				p_dest[0] = p_src[0];
				p_dest[1] = p_src[1];
			break;
			case MM_LEFT_CHANNEL_TO_STEREO:
				p_dest[0] = p_src[0];
				p_dest[1] = p_src[0];
			break;
			case MM_RIGHT_CHANNEL_TO_STEREO:
				p_dest[0] = p_src[1];
				p_dest[1] = p_src[1];
			break;
			default:
