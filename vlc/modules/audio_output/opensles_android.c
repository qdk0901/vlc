...
#define SetChannelMute(a, b, c) (*a)->SetChannelMute(a, b, c)
...
static int ChannelMuteSet(audio_output_t *aout, int channel, int mute)
{
    if (!aout->sys->muteSoloItf)
        return -1;

    SLresult r = SetChannelMute(aout->sys->muteSoloItf, channel, mute);
    return (r == SL_RESULT_SUCCESS) ? 0 : -1;
}
...
static int Start(audio_output_t *aout, audio_sample_format_t *restrict fmt)
{
    ...
    if (is_quad_channel) {
        format_pcm.numChannels      = 4;
        format_pcm.channelMask      = SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT | SL_SPEAKER_BACK_LEFT | SL_SPEAKER_BACK_RIGHT;           
    } else {
    ...
    //create audio player
    const SLInterfaceID ids2[] = { sys->SL_IID_ANDROIDSIMPLEBUFFERQUEUE, sys->SL_IID_VOLUME, sys->SL_IID_MUTESOLO };
	...
	result = GetInterface(sys->playerObject, sys->SL_IID_MUTESOLO, &sys->muteSoloItf);
    CHECK_OPENSL_ERROR("failed to get solo interface.");
...
#define OPENSL_DLSYM(dest, name)                       \
	...
	OPENSL_DLSYM(SL_IID_MUTESOLO, "MUTESOLO");
...
#undef OPENSL_DLSYM
	...
	aout->channel_mute_set   = ChannelMuteSet;
