...
void libvlc_audio_set_channel_mute( libvlc_media_player_t *mp, int channel, int mute )
{
    audio_output_t *aout = GetAOut( mp );
    if( aout != NULL )
    {
        mute = aout_ChannelMuteSet( aout, channel, mute );
        vlc_object_release( aout );
    }
}

void libvlc_audio_set_music_mode( libvlc_media_player_t *mp, int type)
{
    audio_output_t *aout = GetAOut( mp );
    if( aout != NULL )
    {
        var_SetInteger(aout, "music-mode", type);
        vlc_object_release( aout );
    }
}
