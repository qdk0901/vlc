...
audio_output_t *aout_New (vlc_object_t *parent)
{
    ...
    var_Create (aout, "music-mode", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
...
static int aout_OutputChannelMuteSet (audio_output_t *aout, int channel, int mute)
{
    aout_OutputAssertLocked (aout);
    return (aout->channel_mute_set != NULL) ? aout->channel_mute_set (aout, channel, mute) : -1;
}
...
void aout_OutputUnlock (audio_output_t *aout)
{
	...
	if (owner->req.channel >= 0) {
        aout_OutputChannelMuteSet (aout, owner->req.channel, owner->req.mute);
        owner->req.channel = -1;		
	}
...
int aout_ChannelMuteSet (audio_output_t *aout, int channel, int mute)
{
    aout_owner_t *owner = aout_owner (aout);

    vlc_mutex_lock (&owner->req.lock);
    owner->req.channel = channel;
	owner->req.channel_mute = mute;
    vlc_mutex_unlock (&owner->req.lock);
